package users

import (
	"context"
	"encoding/json"
	"fmt"
	"io/ioutil"
	"net/http"
	"os"
	"strings"
	"time"

	"github.com/MiriConf/miriconf-backend/helpers"
	jwt "github.com/golang-jwt/jwt/v4"
	"go.mongodb.org/mongo-driver/bson"
	"go.mongodb.org/mongo-driver/mongo"
	"go.mongodb.org/mongo-driver/mongo/options"
)

func EditUsers(w http.ResponseWriter, r *http.Request) {
	mongoURI := os.Getenv("MONGO_URI")

	headerToken := r.Header.Get("Authorization")
	if headerToken == "" {
		w.WriteHeader(http.StatusUnauthorized)
		return
	}

	token, err := helpers.ValidateToken(headerToken)
	if err != nil {
		if err == jwt.ErrSignatureInvalid {
			w.WriteHeader(http.StatusUnauthorized)
			return
		}
		w.WriteHeader(http.StatusBadRequest)
		return
	}

	if !token.Valid {
		w.WriteHeader(http.StatusUnauthorized)
		return
	}

	status, userID := helpers.GetRequestID("user", r, w)
	if status == 1 {
		return
	}

	reqBody, err := ioutil.ReadAll(r.Body)
	if err != nil {
		error := helpers.ErrorMsg("error reading request body")
		w.Write(error)
		helpers.EndpointError("error reading request body", r)
		return
	}

	var putUser User
	json.Unmarshal(reqBody, &putUser)

	if strings.TrimSpace(putUser.Fullname) == "" {
		error := helpers.ErrorMsg("no fullname in request")
		w.Write(error)
		helpers.EndpointError("no fullname in request", r)
		return
	}

	if strings.TrimSpace(putUser.Email) == "" {
		error := helpers.ErrorMsg("no email in request")
		w.Write(error)
		helpers.EndpointError("no email in request", r)
		return
	}

	client, err := mongo.Connect(context.TODO(), options.Client().ApplyURI(mongoURI))
	if err != nil {
		panic(err)
	}
	defer func() {
		if err = client.Disconnect(context.TODO()); err != nil {
			panic(err)
		}
	}()

	coll := client.Database("miriconf").Collection("users")

	var nameCheck bson.M
	err = coll.FindOne(context.TODO(), bson.D{{Key: "email", Value: putUser.Email}}).Decode(&nameCheck)
	if err != mongo.ErrNoDocuments {
		error := helpers.ErrorMsg("user with this email already exists")
		w.Write(error)
		helpers.EndpointError("user with this email already exists", r)
		return
	}

	createdAt := time.Now()

	doc := bson.D{{"$set", bson.D{{Key: "fullname", Value: strings.TrimSpace(putUser.Fullname)}, {Key: "email", Value: strings.TrimSpace(putUser.Email)}, {Key: "createdat", Value: createdAt.Format("01-02-2006 15:04:05")}}}}
	filter := bson.D{{Key: "_id", Value: userID}}

	result, err := coll.UpdateOne(context.TODO(), filter, doc)
	if err != nil {
		panic(err)
	}

	documentID := fmt.Sprintf("%v", result.UpsertedID)
	success := helpers.SuccessMsg(putUser.Username + " successfully updated at " + documentID)
	w.Write(success)
	helpers.SuccessLog(r)
	return
}
